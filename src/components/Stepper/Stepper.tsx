...
function Item({ text }: { text: string }) {
  ...
  return (
    <div className="flex items-center space-x-2 mb-2 first:mt-3.5 last:mb-3.5">
...
export function Stepper({ steps }: StepperProps) {
  ...
  const circleClasses = useCallback(
    (index: number) => {
      let stateClass;
      switch (themeColor) {
  ...
  const connectorClasses = useCallback(
    (index: number) => {
      let secondaryClass;
      switch (themeColor) {
        case ThemeColors.PRIMARY:
  ...
  return (
    <>
      <div className="relative flex justify-between flex-col xl:flex-row">
        {steps.map((step, index) => (
          ...
          >
            ...
            <div className="font-roboto-flex font-semibold text-grey-100 text-md mt-12 uppercase xl:text-base xl:text-md">
              {step.label}
            </div>
            <div className="font-roboto-flex text-grey-300 text-xs mt-3">
            ...
            {index < steps.length - 1 ? (
              <div className={connectorClasses(index)}></div>
            ) : null}
          </div>
        ))}
      </div>
